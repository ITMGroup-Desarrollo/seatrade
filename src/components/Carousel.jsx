...
export default function Carousel() {
  ...
  const initializeSwiper = () => {
    ...
    const getSlidesPerView = () => (window.innerWidth >= 1600 ? slides : slides - 2);
  ...
  return (
    <div className="grid grid-rows-1 grid-cols-12 h-full px-8 py-4">
      ...
      >
        ...
        ">
          <p key={eventKey} className="text-xs 3xl:text-xl font-semibold animate-fadeIn">{port.instagram}</p>
          <div>
            <img key={eventKey} className="animate-fadeIn" src={port.qr} width={70} alt="" />
          </div>
          <p className="uppercase text-xs 3xl:text-xl font-semibold animate-fadeIn">Follow Us!</p>
        </Card>
      </div>
    </div>
  );
